    ...
    return <div ref={containerRef} className='relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3'>
        {
            isInView ? 
            <>
                <ReviewColumn reviews={[...column1, ...columns3.flat(), ...column2]} reviewClassName={(reviewIndex) => cn({ "md:hidden":reviewIndex >= column1.length + columns3[0].length, "lg:hidden": reviewIndex >= column1.length })} msPerPixel={10}/>    
                <ReviewColumn className='hidden md:block ' reviews={[...column2, ...columns3[1]]} reviewClassName={(reviewIndex) => reviewIndex >= column2.length ? "lg:hidden" : ""} msPerPixel={15}/>    
                <ReviewColumn className='hidden md:block ' reviews={columns3.flat()} msPerPixel={10}/>    
            </>
            : null
        }
